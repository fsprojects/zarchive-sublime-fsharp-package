...
class Test_FSharpFile(unittest.TestCase):
    ...
    def testCannotDetectCodeFileBasedOnlyOnSyntaxDef(self):
        v = self.win.new_file()
        v.set_syntax_file('Packages/FSharp/FSharp.tmLanguage')
        file_info = FileInfo(v)
        self.assertFalse(file_info.is_fsharp_code)
    ...
    def testCanDetectCodeFile(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            time.sleep(0.01)
            file_info = FileInfo(v)
            self.assertTrue (file_info.is_fsharp_code_file)
    ...
    def testCanDetectCodeForCodeFile(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            time.sleep(0.01)
            file_info = FileInfo(v)
            self.assertTrue (file_info.is_fsharp_code)
    ...
    def testCanDetectProjectFile(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            time.sleep(0.01)
            file_info = FileInfo (v)
            self.assertTrue (file_info.is_fsharp_project_file)


class Test_FSharpFile_path(unittest.TestCase):
    def setUp(self):
        self.win = sublime.active_window()
    ...
    def testCannotFindPathIfNone(self):
        v = self.win.new_file()
        v.set_syntax_file('Packages/FSharp/FSharp.tmLanguage')
        file_info = FileInfo(v)
        self.assertEquals(file_info.path, None)
